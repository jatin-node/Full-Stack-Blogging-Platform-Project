...
const UserNavigationPanel = () => {
  ...
  return (
    <AnimationWrapper transition={{ duration: 0.2 }}>
      <div className="w-64 bg-white absolute right-0 border border-gray-100 overflow-hidden duration-200">
        {auth?.user?.username && (
          <>
            <Link
              to={`${auth.user.username}/editor`}
              className="flex sm:hidden gap-2 link pl-8 py-4 font-semibold"
            >
